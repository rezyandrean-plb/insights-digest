...
type LaunchCategory = NewLaunchItem["category"];

const CATEGORIES: LaunchCategory[] = ["Most Viewed", "Latest", "Editor's Pick"];

const ITEMS_PER_PAGE = 10;

const categoryBadgeClass: Record<LaunchCategory, string> = {
    "Most Viewed": "bg-[#e8f0fe] text-[#1a56db]",
    Latest: "bg-[#ecfdf5] text-[#059669]",
    "Editor's Pick": "bg-[#fff7ed] text-[#c2410c]",
};

const emptyItem: Omit<NewLaunchItem, "id"> = {
    slug: "",
    title: "",
    excerpt: "",
    image: "",
...
export default function AdminNewLaunchesPage() {
    ...
    const [filterCategory, setFilterCategory] = useState<LaunchCategory | "All">("All");
    const [currentPage, setCurrentPage] = useState(1);
    const [heroItem, setHeroItem] = useState<NewLaunchItem | null>(null);
    const [dialogOpen, setDialogOpen] = useState(false);
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<NewLaunchItem | null>(null);
    const [deletingItem, setDeletingItem] = useState<NewLaunchItem | null>(null);
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            setError("Failed to load new launch series from server.");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const setHero = useCallback(
        async (itemId: string) => {
            ...
            setError(null);
            try {
                const res = await fetch("/api/new-launches/hero", {
                    method: "PUT",
    ...
    const openEditDialog = useCallback((item: NewLaunchItem) => {
        setEditingItem(item);
        setFormData({
            slug: item.slug,
            title: item.title,
    ...
    const openDeleteDialog = useCallback((item: NewLaunchItem) => {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            if (editingItem) {
                const res = await fetch("/api/new-launches", {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
    ...
    const handleDelete = useCallback(async () => {
        ...
        try {
            const res = await fetch(`/api/new-launches?id=${deletingItem.id}`, {
                method: "DELETE",
            });
            ...
            <div className="py-10 sm:py-14 lg:py-16">
                <div className="container-custom flex items-center justify-center py-32">
                    <div className="flex flex-col items-center gap-3 text-muted">
                        ...
                        <span className="text-sm">Loading new launch series...</span>
                    </div>
                </div>
            </div>
        );
    ...
    return (
        <div className="py-10 sm:py-14 lg:py-16">
            <div className="container-custom">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                    <div>
                        ...
                        <h1 className="text-2xl sm:text-3xl font-bold text-foreground">
                            Manage New Launch Series
                        </h1>
                        <p className="text-sm text-muted mt-1">
                            {itemsList.length} items total
                        </p>
                    </div>
                    <button
                ...
                {/* Hero card - manage hero content for /all-new-launch-series */}
                <div className="mb-8 rounded-2xl border border-border/50 bg-white p-5 sm:p-6 shadow-sm">
                    ...
                    <p className="text-sm text-muted mb-4">
                        This item is shown in the hero section on the New Launch Series page. Choose an item below to set as hero.
                    </p>
                    {heroItem ? (
                ...
                <div className="flex flex-col sm:flex-row gap-3 mb-6">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            setFilterCategory(e.target.value as LaunchCategory | "All");
                            setCurrentPage(1);
                        }}
                                                ...
                                                <td className="py-3 px-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div className="min-w-0">
                                                            <p className="text-sm font-medium text-foreground truncate max-w-[260px] lg:max-w-[380px]">
                                                                {item.title}
                                                            </p>
                                                            <p className="text-xs text-muted truncate max-w-[260px] lg:max-w-[380px]">
                                                                {item.excerpt}
                                                            </p>
                                                        </div>
                                                    </div>
                        ...
                        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[95vw] max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-2xl shadow-xl p-6 sm:p-8 data-[state=open]:animate-in data-[state=open]:fade-in data-[state=open]:zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:zoom-out-95">
                            <div className="flex items-center justify-between mb-6">
                                <Dialog.Title className="text-xl font-bold text-foreground">
                                    {editingItem ? "Edit Launch Item" : "New Launch Item"}
                                </Dialog.Title>
                            ...
                            <div className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Brief description of the launch item..."
                                        rows={3}
                                    ...
                                    <div>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) =>
                                                updateField(
                                                    "category",
                                                    e.target.value as LaunchCategory
                                                )
                                            }
